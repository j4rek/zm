...
class webpay extends consultas{
	...
	final public function getOrdenCompra($parametro_unico){
		if(isset($parametro_unico) && $parametro_unico != ""){
			return $this->resultado = ("OC_".$parametro_unico."_".($this->anno).($this->mes).($this->dia).($this->hora).($this->minuto).($this->segundo)); 
		}else{
			return $this->resultado = ("OC_".($this->anno).($this->mes).($this->dia).($this->hora).($this->minuto).($this->segundo)); 
		}
	}
	#Metodo que genera el ID de sesion.-
	final public function getIdSSesion(){
		return $this->resultado = ("SS_".($this->hora).($this->minuto).($this->segundo).($this->anno).($this->mes).($this->dia)); 
	}
					...
					$this->resultado		= "<div id='detalle_pago'>
													...
													<table width='100%' cellspacing='0' cellpadding='0' class='General'>
														...
														<tbody>
															<tr class='center'>
																<td class='pieIni'>
																	<a href='http://www.buscatupega.cl/aviso.html/id/".$tbk_id_aviso."/".$titulo_aviso."/'>Ver Aviso...</a>
																</td>
																<td>$ ".$tbk_monto."</td>
			...
			if(isset($codigo) && $codigo != ""){
				switch($codigo){
					...
					case -1:
						...
						break;
					case -2:
						$respuesta = "Transaccion debe reitentarse.";
						break;
					case -3:
			...
			if(isset($orden_compra) && $orden_compra != "" && isset($nombre_cliente) && $nombre_cliente != ""){
				...
				<html>
					...
					<body style='margin:0px;padding:0px;background-color:#efefef;'>
						...
						<img src='http://usados.zoomautomotriz.com/img/mail_header.jpg' border='0' width='488' height='83' alt='ZoomAutomotriz.com' /></div>
						...
						<p>Si lo desea, puede modificar su aviso totalmente gratis, haciendo <a href=\"http://usados.zoomautomotriz.com/editar_2.html/ID/".$datos[idAviso]."/CODE/".$datos[contrasena]."\" style=\"text-decoration:none;\">click aqu&iacute;</a>.
						</p>
						</span>
						<span style='font: normal 12px Tahoma, Arial, Verdana, sans-serif;color:#666; text-align:justify;'>
						<br /><br />
						...
						<img src='http://usados.zoomautomotriz.com/img/mail_footer.jpg' border='0' width='488' height='42' alt='ZoomAutomotriz.com' /></div>
						</div></center>
						</div>
					</body>
				</html>";
