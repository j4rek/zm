/* 	No tocar... archivo que maneja el auto-completado de cajas de busqueda de la 
	Home. Modificaciones by Kyron
*/
(function(){
...
var TextboxList2 = new Class({
  ...
  initialize: function(element, options){
	...
	enablePlugin: function(name, options){
		this.plugins[name] = new TextboxList2[name.camelCase().capitalize()](this, options);
	},
...
TextboxListBit2.Editable = new Class({
...
TextboxList.Autocomplete = new Class({
	...
	options: {
		...
		remote: {
			...
			param: 'search',
			extraParams: {},
			loadPlaceholder: 'Espere...'
		},
		method: 'standard',
		placeholder: 'Escriba y seleccione el listado de oficios que desea buscar...'
	},
	...
	search: function(bit){
		...
		//window.alert(this.options.unique);
//		myElement = $('oficios');
		//window.alert(this.currentRequest);
		if (this.values.length){
			this.showResults(search);
			//document.formulario.comunas2.value="hola";
		}
	},
	...
	addResult: function(r, search){
		...
		if (this.options.mouseInteraction){
			...
			if (!this.options.onlyFromValues) 
				element.addEvent('mouseleave', function(){ if (this.current == element) this.blur(); }.bind(this));	
		}
	},
	...
	hide: function(ev){
		...
		this.hidetimer = (function(){
			this.hidePlaceholder();
			this.list.setStyle('display', 'none');
			document.formulario.oficios2.value = this.currentSearch;
...
TextboxList2.Autocomplete = new Class({
	...
	options: {
		...
		method: 'standard',
		placeholder: 'Escriba y seleccione el listado de comunas en donde desea buscar...'
	},
...
//		myElement = $('oficios');
	...
	hide: function(ev){
		...
		this.hidetimer = (function(){
			this.hidePlaceholder();
			this.list.setStyle('display', 'none');
			document.formulario.comunas2.value = this.currentSearch;
...
TextboxList2.Autocomplete.Methods = {
