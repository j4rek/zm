...
//BotonPago Class
//Versión 1.1 
//Implementación de nueva clase Botón de Pago
//Fecha última Modificación: 24-10-2011
...
class BotonPago
{
	...
	$this->GeneraLog('1', 'IdSubTrx '.$IdSubTrx);
	...
	foreach ($this->arrayOrdenamiento as $key => $val) {		
			switch($key)
			{
				...
				case "IdSubTrx":
					$datos = $datos.$IdSubTrx;		
				break;	
				case "CodigoIdentificador":
					$datos = $datos.$CodigoIdentificador;
				break;
				case "Boleta":	
					$datos = $datos.$Boleta;
				break;
	...
	// 		$texto = mensaje de log
 	public function GeneraLog($numero, $texto){
		...
		$realtime =  $matriz_ini['rutaLog']['ruta'].date("Ymd").".log"; 
